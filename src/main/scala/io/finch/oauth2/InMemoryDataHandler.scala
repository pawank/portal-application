...
object InMemoryDataHandler extends DataHandler[OAuthUser] {
  ...
  private[this] val accessTokens =
    new ConcurrentHashMap[String, AccessToken]().asScala
  private[this] val authInfosByAccessToken =
    new ConcurrentHashMap[String, AuthInfo[OAuthUser]]().asScala
  ...
  override def findUser(
    ...
    clients.find {
      case ad => username.equals(ad.username) && password.equals(ad.password)
    } match {
  ...
  override def refreshAccessToken(
    ...
    accessTokens.find {
      case (id, at) => at.refreshToken.exists(_.equals(refreshToken))
    } match {
  ...
  override def findAccessToken(token: String): Future[Option[AccessToken]] = {
    Future.value(accessTokens.values.find { at: AccessToken =>
      at.token.equals(token)
    })
  }
